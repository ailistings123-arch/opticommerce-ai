...
function SignupForm() {
  ...
  const handleEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      trackEvent('sign_up', { method: 'email', user_id: u.uid });
      if (inviteToken) router.push('/api/accept-invite?token=' + inviteToken);
      else router.push('/dashboard');
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      trackEvent('sign_up', { method: 'google', user_id: u.uid });
      if (inviteToken) router.push('/api/accept-invite?token=' + inviteToken);
      else router.push('/dashboard');
        ...
        <div className="relative z-10 space-y-8">
          <div>
            ...
            <p className="text-blue-100 text-lg leading-relaxed">
              {inviteToken ? 'Create your account to join the Enterprise workspace.' : 'Join thousands of sellers who use AI to write listings that rank higher and convert better.'}
            </p>
          </div>
        ...
        <div className="w-full max-w-md">
          ...
          {inviteToken && (
            <div className="mb-6 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 flex items-center gap-2">
              ...
              <p className="text-sm text-blue-700">You have a workspace invitation. Create your account to get Enterprise access automatically.</p>
            </div>
          )}

          <h1 className="text-3xl font-bold text-gray-900 mb-1">Create your account</h1>
          <p className="text-gray-500 mb-8">{inviteToken ? 'Sign up to accept your Enterprise invite' : 'Start optimizing your listings for free'}</p>
          ...
          <p className="mt-6 text-center text-sm text-gray-600">
            ...
            <Link href={inviteToken ? '/login?invite=' + inviteToken : '/login'} className="text-blue-600 hover:text-blue-700 font-semibold">Log in</Link>
          </p>
        </div>
      </div>
    </div>
